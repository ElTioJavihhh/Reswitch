...
class ResolutionSwitcherApp(ctk.CTk):
    """
    Clase principal de la aplicación. Actúa como el controlador en el patrón MVC.
    Gestiona el estado, la lógica de negocio y la interacción entre las vistas.
    """
    def __init__(self):
        super().__init__()
        ...
        self.monitors = self._get_available_monitors()
        self.monitor_names = list(self.monitors.keys())
        self.supported_resolutions_cache: Dict[str, List[str]] = {}
        self.original_resolution: Optional[str] = None
        self.game_profiles: List[GameProfile] = []
        self.config: Dict[str, Any] = {}
        
        self.process_checker_thread: Optional[threading.Thread] = None
    ...
    def load_settings(self):
        ...
        try:
            ...
            logger.info(f"Configuración cargada desde {SETTINGS_FILE}")
        except (json.JSONDecodeError, IOError, FileNotFoundError):
            self.config = {}
            logger.warning("No se encontró archivo de configuración, se usarán valores por defecto.")
    ...
    def save_settings(self):
        # Asegurarse de que los frames existen antes de acceder a ellos
        if "settings" not in self.frames or "home" not in self.frames:
            return
        ...
        selected_appearance = settings_view.appearance_mode_selector.get()
        self.config["appearance_mode"] = appearance_map_rev.get(selected_appearance, "System")
        ...
        self.config["tray_notification_shown"] = self.config.get("tray_notification_shown", False)
        ...
        try:
            ...
            logger.info(f"Configuración guardada en {SETTINGS_FILE}")
        except Exception as e:
            self.update_status("status_startup_error", e=e)
    ...
    def _create_widgets(self):
        ...
        self.nav_buttons = {
            "home": widgets.NavButton(self.nav_frame, "nav_home", lambda: self.show_frame("home"), self.lang, self.theme),
            "profiles": widgets.NavButton(self.nav_frame, "nav_profiles", lambda: self.show_frame("profiles"), self.lang, self.theme),
            "settings": widgets.NavButton(self.nav_frame, "nav_settings", lambda: self.show_frame("settings"), self.lang, self.theme),
        }
    ...
    def update_ui_language(self):
        ...
        settings_view.language_selector.set(lang_map_rev.get(self.lang.language, "English"))
        ...
        appearance_map = self._get_translated_appearance_map()
        settings_view.appearance_mode_selector.configure(values=list(appearance_map.values()))
        current_mode_key = self.config.get("appearance_mode", "System")
        settings_view.appearance_mode_selector.set(appearance_map.get(current_mode_key, "System"))
        ...
        if self.monitor_names:
            ...
            if target_monitor in self.monitor_names:
                settings_view.monitor_selector.set(target_monitor)
            else:
                settings_view.monitor_selector.set(self.monitor_names[0])
        self.on_monitor_change(settings_view.monitor_selector.get())

        home_view.desktop_res_selector.set(self.config.get("desktop_res"))
        home_view.game_res_selector.set(self.config.get("game_res"))
        settings_view.start_win_var.set(self.config.get("start_with_windows", False))
        settings_view.desktop_hotkey_entry.configure(text=self.config.get("hotkey_desktop"))
        settings_view.game_hotkey_entry.configure(text=self.config.get("hotkey_game"))
    ...
    def update_theme_for_all_widgets(self):
        ...
        for view in self.frames.values():
            if hasattr(view, 'winfo_children'):
                for widget in view.winfo_children():
                    self._apply_theme_to_widget(widget, theme)
        
        if "profiles" in self.frames and hasattr(self.frames["profiles"], "refresh_profiles_list"):
            self.frames["profiles"].refresh_profiles_list()

    def _apply_theme_to_widget(self, widget, theme):
        if not widget.winfo_exists():
            return

        profiles_list_frame = getattr(self.frames.get("profiles"), "profiles_list_frame", None)
        home_subtitle_label = getattr(self.frames.get("home"), "home_subtitle_label", None)
        desktop_hotkey_entry = getattr(self.frames.get("settings"), "desktop_hotkey_entry", None)
        game_hotkey_entry = getattr(self.frames.get("settings"), "game_hotkey_entry", None)

        if isinstance(widget, (ctk.CTkFrame, ctk.CTkScrollableFrame)):
            if widget != profiles_list_frame:
                widget.configure(fg_color=theme.get('frame_bg', theme['bg']))

        for child in widget.winfo_children():
            self._apply_theme_to_widget(child, theme)

        if isinstance(widget, ctk.CTkLabel):
            if widget == home_subtitle_label:
                widget.configure(text_color=theme['text_secondary'])
            else:
                widget.configure(text_color=theme['text'])
        elif isinstance(widget, ctk.CTkButton) and not isinstance(widget, widgets.NavButton):
            if widget in [desktop_hotkey_entry, game_hotkey_entry]:
                widget.configure(fg_color=theme['button'], text_color=theme['text'], hover_color=theme['button_hover'])
            else:
                widget.configure(fg_color=theme['accent'], text_color=theme['text_on_accent'], hover_color=theme['accent_hover'])
        elif isinstance(widget, ctk.CTkOptionMenu):
            widget.configure(fg_color=theme['frame_bg'], button_color=theme['button'], button_hover_color=theme['button_hover'], text_color=theme['text'])
        elif isinstance(widget, ctk.CTkCheckBox):
            widget.configure(fg_color=theme['accent'], checkmark_color=theme['text_on_accent'], text_color=theme['text'])
            ...
            while True:
                try:
                    ...
                    if device.StateFlags & win32con.DISPLAY_DEVICE_ACTIVE:
                        monitor_name = f"Monitor {i+1}: {device.DeviceString}"
                        monitors[monitor_name] = device.DeviceName
                    i += 1
                except win32api.error:
                    break
        except Exception as e:
            logger.error(f"Error enumerando monitores: {e}")
        if not monitors:
            monitors["Principal"] = None
        return monitors
    ...
    def set_resolution(self, res_str: str, monitor_name: Optional[str] = None):
        if not monitor_name:
            monitor_name = self.config.get("target_monitor")
        device_name = self.monitors.get(monitor_name)
        if not res_str or 'x' not in res_str:
            logger.warning(f"Formato de resolución inválido: {res_str}")
            return
        try:
            ...
            logger.info(f"Resolución cambiada a {res_str} en {monitor_name}")
        except Exception as e:
            logger.error(f"Fallo al cambiar la resolución a {res_str}: {e}", exc_info=True)
            self.update_status("status_res_unsupported", res=res_str)

    def _get_supported_resolutions(self, monitor_name: Optional[str]) -> List[str]:
        if not monitor_name or monitor_name in self.supported_resolutions_cache:
            return self.supported_resolutions_cache.get(monitor_name, [])
        ...
        try:
            while True:
                ...
                w, h = devmode.PelsWidth, devmode.PelsHeight
                resolutions.add(f"{w}x{h} {helpers.calculate_aspect_ratio(w, h)}")
                i += 1
        except (win32api.error, AttributeError):
            pass 
        
        if not resolutions:
            sorted_res = ["1920x1080 (16:9)", "1280x720 (16:9)"]
        else:
            sorted_res = sorted(list(resolutions), key=lambda r: tuple(map(int, r.split(' ')[0].split('x'))), reverse=True)
        
        self.supported_resolutions_cache[monitor_name] = sorted_res
        return sorted_res

    def setup_hotkeys(self):
        try:
            keyboard.unhook_all()
            desktop_hotkey = self.config.get("hotkey_desktop")
            game_hotkey = self.config.get("hotkey_game")
            if desktop_hotkey:
                keyboard.add_hotkey(desktop_hotkey, lambda: self.set_resolution(self.config.get("desktop_res")))
            if game_hotkey:
                keyboard.add_hotkey(game_hotkey, lambda: self.set_resolution(self.config.get("game_res")))
            logger.info("Atajos de teclado configurados.")
        except Exception as e:
            self.update_status("status_hotkey_error", e=e)
            logger.error(f"Error al configurar atajos de teclado: {e}")

    def check_for_game_process(self):
        game_is_running = False
        active_profile: Optional[GameProfile] = None
        while not self.stop_thread.is_set():
            time.sleep(2)
            found_profile: Optional[GameProfile] = None
            profile_executables = {os.path.basename(p.path): p for p in self.game_profiles}
            
            for proc in psutil.process_iter(['name']):
                if proc.info['name'] in profile_executables:
                    found_profile = profile_executables[proc.info['name']]
                    break
            ...
            if found_profile and found_profile != active_profile:
                ...
                logger.info(f"Juego '{os.path.basename(active_profile.path)}' detectado. Resolución original: {self.original_resolution}")
                
                target_res = active_profile.res
                if self._get_current_resolution_str(monitor) != target_res:
                    logger.info(f"Cambiando a resolución de juego: {target_res}")
                    self.after(0, self.set_resolution, target_res, monitor)

            elif not found_profile and game_is_running and active_profile:
                game_is_running = False
                logger.info(f"Juego '{os.path.basename(active_profile.path)}' cerrado. Restaurando resolución.")
                monitor = active_profile.monitor or self.config.get("target_monitor")
                if self.original_resolution:
                    self.after(0, self.set_resolution, self.original_resolution, monitor)
                active_profile = None
    ...
    def start_process_checker(self):
        ...
        logger.info("Monitor de procesos de juego iniciado.")

    def on_monitor_change(self, monitor_name: str):
        resolutions = self._get_supported_resolutions(monitor_name)
        home_view = self.frames["home"]
        
        if not resolutions: return

        home_view.desktop_res_selector.configure(values=resolutions)
        home_view.game_res_selector.configure(values=resolutions)
        
        if home_view.desktop_res_selector.get() not in resolutions:
            home_view.desktop_res_selector.set(resolutions[0])
            
        if home_view.game_res_selector.get() not in resolutions:
            home_view.game_res_selector.set(resolutions[1] if len(resolutions) > 1 else resolutions[0])

    def change_language(self, choice: str):
        lang_map = {"English": "en", "Español": "es", "中文": "zh"}
        self.lang.set_language(lang_map.get(choice, "en"))
        self.update_ui_language()

    def change_appearance_mode(self, new_mode_translated: str):
        lang_map_rev = {v: k for k, v in self._get_translated_appearance_map().items()}
        new_mode_key = lang_map_rev.get(new_mode_translated, "System")
        ctk.set_appearance_mode(new_mode_key)
        self.update_theme_for_all_widgets()

    def _get_translated_appearance_map(self) -> Dict[str, str]:
        return {
            "Light": self.lang.get("theme_light"),
            "Dark": self.lang.get("theme_dark"),
            "System": self.lang.get("theme_system")
        }

    def apply_theme(self):
        mode = ctk.get_appearance_mode()
        self.theme = THEMES.get(mode, THEMES["Light"])

    def open_profile_editor(self, profile_to_edit: Optional[GameProfile] = None):
        if hasattr(widgets, 'ProfileEditor'):
            editor = widgets.ProfileEditor(master=self, controller=self, profile_to_edit=profile_to_edit)
            editor.grab_set()

    def delete_profile(self, profile_to_delete: GameProfile):
        if profile_to_delete in self.game_profiles:
            self.game_profiles.remove(profile_to_delete)
            self.save_settings()
            if "profiles" in self.frames:
                self.frames["profiles"].refresh_profiles_list()

    def update_status(self, key: str, **kwargs):
        message = self.lang.get(key, **kwargs)
        level = "error" if "error" in key.lower() else ("success" if "saved" in key or "changed" in key else "info")
        color = self.theme.get(f"text_{level}", self.theme['text_secondary'])
    ...
    def quick_switch(self):
        monitor = self.config.get("target_monitor")
        current_res = self._get_current_resolution_str(monitor)
        desktop_res = self.config.get("desktop_res")
        game_res = self.config.get("game_res")
        
        target_res = game_res if current_res == desktop_res else desktop_res
        self.set_resolution(target_res, monitor)
    ...
    def apply_startup_setting(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            # En desarrollo, la ruta correcta es al lanzador principal, run.py
        ...
        try:
            with reg.OpenKey(reg.HKEY_CURRENT_USER, key_path, 0, reg.KEY_ALL_ACCESS) as key:
                if self.config.get("start_with_windows"):
                    reg.SetValueEx(key, APP_NAME, 0, reg.REG_SZ, command)
                    logger.info(f"Aplicación añadida al inicio: {command}")
                else:
                    try:
                        reg.DeleteValue(key, APP_NAME)
                        logger.info("Aplicación eliminada del inicio.")
                    except FileNotFoundError:
                        pass
        except Exception as e:
            self.update_status("status_startup_error", e=e)
            logger.error(f"Error al modificar el registro de inicio: {e}")

    def capture_hotkey(self, button_widget: ctk.CTkButton):
        if hasattr(widgets, 'HotkeyCaptureWindow'):
            capture_window = widgets.HotkeyCaptureWindow(master=self, controller=self, button_to_update=button_widget)
            capture_window.grab_set()

    def open_game_scanner_window(self):
        if hasattr(widgets, 'GameScannerWindow'):
            scanner = widgets.GameScannerWindow(master=self, controller=self)
            scanner.grab_set()
    ...
    def hide_to_tray(self):
        ...
        if not self.config.get("tray_notification_shown", False):
    ...
    def quit_app(self):
        ...
        if self.tray_icon:
            self.tray_icon.stop()
        if self.process_checker_thread:
