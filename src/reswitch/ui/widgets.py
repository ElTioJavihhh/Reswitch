...
# --- CORRECCIÓN 1: Se importa la clase GameProfile directamente ---
from ..core.models import GameProfile

# --- WIDGETS DE NAVEGACIÓN Y ETIQUETAS (Sin cambios importantes) ---
...
# --- CORRECCIÓN 3: Nuevo widget para la barra de información ---
class InfoBanner(ctk.CTkFrame):
    def __init__(self, master: Any, text_key: str, lang_manager: Any, theme: Dict[str, str], icon_path: str):
        ...
        self.text_label = ctk.CTkLabel(self, text=self.lang_manager.get(text_key), font=("Segoe UI", 13), text_color=theme.get('text_secondary'), wraplength=450, justify="left")
        self.text_label.pack(side="left", padx=(0, 10), pady=10, fill="x", expand=True)

    def update_language(self):
        self.text_label.configure(text=self.lang_manager.get(self.text_key))

# --- VENTANAS EMERGENTES (TOPLEVELS) CON CORRECCIONES ---
class ProfileEditor(ctk.CTkToplevel):
    def __init__(self, master: Any, controller: Any, profile_to_edit: Optional[GameProfile] = None):
        ...
        title = self.lang.get("profile_editor_title_edit") if profile_to_edit else self.lang.get("profile_editor_title_new")
        self.title(title)
        self.geometry("500x350")
        self.wm_iconbitmap(controller.icon_path) # <--- CORRECCIÓN 2: Uso de wm_iconbitmap
        self.transient(master)
        self.configure(fg_color=self.theme['bg'])
        # ... (resto del código de __init__ se mantiene igual)
        ...
        self.path_entry = ctk.CTkEntry(path_frame, font=self.controller.font_main, fg_color=self.theme['frame_bg'], border_color=self.theme['frame_border'], text_color=self.theme['text'])
        self.path_entry.pack(side="left", fill="x", expand=True)
        browse_button = ctk.CTkButton(path_frame, text="...", width=30, font=self.controller.font_main, command=self._browse_file)
        ...
        self.monitor_selector = ctk.CTkOptionMenu(self, values=self.controller.monitor_names, font=self.controller.font_main, command=self._update_res_for_monitor)
        self.monitor_selector.pack(fill="x", padx=20, pady=5)
        ...
        self.res_selector = ctk.CTkOptionMenu(self, values=self.controller._get_supported_resolutions(initial_monitor), font=self.controller.font_main)
        self.res_selector.pack(fill="x", padx=20, pady=5)
        save_button = ctk.CTkButton(self, text=self.lang.get("save_profile_button"), height=40, font=self.controller.font_bold, command=self._save_profile)
        ...
        if self.profile_to_edit:
            self.path_entry.insert(0, self.profile_to_edit.path)
            monitor = self.profile_to_edit.monitor or initial_monitor
            self.monitor_selector.set(monitor)
            self._update_res_for_monitor(monitor)
            self.res_selector.set(self.profile_to_edit.res)
    ...
    def _save_profile(self):
        ...
        res = self.res_selector.get()
        monitor = self.monitor_selector.get()
        if path and res:
            profile_data = {"path": path, "res": res, "monitor": monitor, "name": os.path.basename(path)}
            if self.profile_to_edit:
                self.profile_to_edit.path, self.profile_to_edit.res, self.profile_to_edit.monitor, self.profile_to_edit.name = path, res, monitor, os.path.basename(path)
            else:
                # --- CORRECCIÓN 1: Se llama a GameProfile directamente ---
                self.controller.game_profiles.append(GameProfile(**profile_data))
            
            self.controller.save_settings()
            self.controller.frames["profiles"].refresh_profiles_list()
            self.destroy()
...
class GameScannerWindow(ctk.CTkToplevel):
    ...
    def _add_selected_games(self):
        default_res_list = self.controller._get_supported_resolutions(self.controller.config.get("target_monitor"))
        default_res = default_res_list[0] if default_res_list else ""
        
        for cb in self.checkboxes:
            exe_path = cb.get()
            if exe_path:
                # --- CORRECCIÓN 1: Se llama a GameProfile directamente ---
                profile_data = {"path": exe_path, "res": default_res, "monitor": self.controller.config.get("target_monitor"), "name": os.path.basename(exe_path)}
                self.controller.game_profiles.append(GameProfile(**profile_data))
    ...
    # (El resto de los métodos de GameScannerWindow se mantienen igual)
...
# (Las demás clases Toplevel también deben usar wm_iconbitmap)
class HotkeyCaptureWindow(ctk.CTkToplevel):
    def __init__(self, master: Any, controller: Any, button_to_update: ctk.CTkButton):
        super().__init__(master)
        self.wm_iconbitmap(controller.icon_path) # <--- CORRECCIÓN 2
        # ... (resto del código de la clase se mantiene igual)
        ...
        self.title(self.lang.get("hotkey_capture_title"))
...
class TrayNotificationDialog(ctk.CTkToplevel):
    def __init__(self, master: Any, controller: Any, on_close_callback: Callable):
        ...
        self.title(self.lang.get("tray_notice_title"))
        self.transient(master)
        self.configure(fg_color=self.theme['bg'])
        main_x, main_y, main_w, main_h = master.winfo_x(), master.winfo_y(), master.winfo_width(), master.winfo_height()
